...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> mapCaloriesSumByTime = new HashMap<>();
        for (UserMeal m : meals) {
            mapCaloriesSumByTime.merge(m.getDateTime().toLocalDate(), m.getCalories(), Integer::sum);
        }
        List<UserMealWithExcess> userMealWithExcesses = new ArrayList<>();
        for (UserMeal m : meals) {
            LocalDateTime localDateTime = m.getDateTime();
            if (isBetweenHalfOpen(localDateTime.toLocalTime(), startTime, endTime)) {
                boolean isExcess = mapCaloriesSumByTime.get(localDateTime.toLocalDate()) > caloriesPerDay;
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> mapCaloriesSumByTime = meals.stream()
        ...
        return meals.stream()
                ...
                .map(m -> new UserMealWithExcess(
                        m.getDateTime(), m.getDescription(), m.getCalories(), mapCaloriesSumByTime.get(m.getDateTime().toLocalDate()) > caloriesPerDay))
                .collect(toList());
